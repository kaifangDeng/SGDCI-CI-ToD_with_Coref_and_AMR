...
class DatasetTool(object):
    ...
    def load_data(data_path):
        ...
        for dialogue_components_item in raw_data:
            ...
            constructed_info, last_response, consistency, kb_lenth = DatasetTool.get_info(dialogue_components_item, domain, sent_lenth)
            ...
            data_item['kb_lenth'] = kb_lenth
            data.append(data_item)
        return data

    @staticmethod
    def get_info(dialogue_components_item, domain, sent_lenth):
        """
        ...
        constructed_info, b = DatasetTool.construct_info(kb_expanded, history_sentences, sent_lenth)

        return constructed_info, last_response, consistency, b
    ...
    def construct_info(kb_expanded, history_sentences, dialogue_len):
        ...
        for row in kb_expanded:
            ...
            # construct_info += ' ; '
            construct_info = construct_info.replace('  ',' ')
            if len(construct_info.split(' ')) > 500 - dialogue_len:
                break
            construct_info += ' [EOK]'

            construct_infos.append(construct_info)

            aa = construct_info.split(' ')
            b = len(aa)

        construct_info1 = ''
        for i, sentence in enumerate(history_sentences):
            if i % 2 == 0:
                construct_info1 += "[USR] " + sentence + ' '
            else:
                construct_info1 += "[SYS] " + sentence + ' '
        ...
        return (construct_infos, construct_info1), 0
